  ...
  <div>
    <div v-if="isLoading">
      <div style="width:100%;height:100vh;display:flex;justify-content:center;align-items:center;">
        <div>
          <v-img :aspect-ratio="1/1" width="66" :src="imgLogo"></v-img>
        </div>
      </div>
    </div>
    <div v-if="!isLoading">
      <div v-if="statusPageItemCode==200">
        <v-container>
            ...
            <v-card-text>
              <div class="display-align-center reset-display" style="justify-content: space-between;"
                ...
                <div class="display-align-center">
                  <v-chip class="ma-2" color="#ccc" text-color="white" v-if="item1.device.log[0].check_success==2">
                    {{item1.device.log[0].check_success|filterDeviceStatus}}
                  </v-chip>
                  <v-chip class="ma-2" color="success" text-color="white" v-if="item1.device.log[0].check_success==1">
                    {{item1.device.log[0].check_success|filterDeviceStatus}}
                  </v-chip>
                  <v-chip class="ma-2" color="red" text-color="white" v-if="item1.device.log[0].check_success==0">
                    {{item1.device.log[0].check_success|filterDeviceStatus}}
                  </v-chip>
      ...
      <div v-if="statusPageItemCode==404"
...
<script>
  ...
  export default {
    ...
    methods: {
      getList() {
        this.isLoading = true;
        axios.get(process.env.VUE_APP_API_URL + "v1/view_status_page", {
          params: {
          ...
          this.statusPageItemCode = res.data.code;
          this.statusPageItem = res.data.data;
          document.title = "柠檬监控 - " + this.statusPageItem.name;
